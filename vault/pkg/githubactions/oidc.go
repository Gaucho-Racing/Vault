...
func (v *Verifier) Verify(ctx context.Context, token string) (Claims, error) {
	...
	return Claims{
		...
		Repository: strings.ToLower(rawClaims.Repository),
		Ref:        rawClaims.Ref,
		Workflow:   rawClaims.Workflow,
		Actor:      rawClaims.Actor,
		RunID:      rawClaims.RunID,
		RunAttempt: rawClaims.RunAttempt,
		SHA:        rawClaims.SHA,
		EventName:  rawClaims.EventName,
...
func (v *Verifier) refreshKeys(ctx context.Context) error {
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("github oidc jwks returned status %d", resp.StatusCode)
	}
	...
	if len(keys) == 0 {
		return errors.New("github oidc jwks did not include usable rsa keys")
	}
...
func (v *Verifier) discoveryJWKSURI(ctx context.Context) (string, error) {
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("github oidc discovery returned status %d", resp.StatusCode)
	}
	...
	if document.JWKSURI == "" {
		return "", errors.New("github oidc discovery did not include jwks_uri")
	}
...
func (v *Verifier) validateClaims(claims tokenClaims) error {
	...
	if strings.TrimSpace(claims.Repository) == "" {
		return invalidToken("repository claim is required")
	}
	if strings.TrimSpace(claims.Ref) == "" {
		return invalidToken("ref claim is required")
	}
	return nil
}
